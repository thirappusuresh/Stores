<div class="row-fluid">
	<div class="span5 well">
		<fieldset><legend>Store In</legend></fieldset>
		...
		<?php $form=$this->beginWidget('bootstrap.widgets.TbActiveForm',array(
			...
			<?php echo $form->dropDownListRow($model,'iid', CHtml::listData(Items::model()->findAll(),'iid','item_name'),array('class'=>'span11 storeitem'));?>
			...
			<div class="control-group" id="uom" style="display:none;">
				<div class="controls storeuom">
				</div>
			</div>
		
			<?php echo $form->textFieldRow($model,'quantity',array('class'=>'span11')); ?>
			...
			<?php echo $form->textAreaRow($model,'received_from',array('rows'=>3, 'cols'=>50, 'class'=>'span11')); ?>
		        ...
		        array('value'=>'$data->rate * $data->quantity', 'header'=>'Total Cost'),
		       	array(
		        	...
		        	'buttons'=>array(
		        		'update'=>
		        			array(
		        				'url'=>'Yii::app()->createUrl("/storeIn/update", array("id"=>$data->sin_id))',
		        			),
		        		'delete'=>
		        			array(
		        				'url'=>'Yii::app()->createUrl("/storeIn/delete", array("id"=>$data->sin_id))',
		        			),
		        	),
		            'htmlOptions'=>array(),
		        ),
...
<script type="text/javascript">
    $(document).ready(function () {
		$('.storeitem').live('change', function() {
		  $.ajax({
		  	type: 'GET',
		  	url: "/storeIn/getUom/id/"+$(this).val(),
		  	beforeSend: function(){
		  		$("#uom").hide();
		  		$("#loading").show();
		    },
		  	success: function(result) {
		  		$("#loading").hide();
		  		$("#uom").show();
		  		$(".storeuom").html("<p class=\"help-block\">UOM: "+result+"</p>");
		  	}
		  })	
		});
    });
